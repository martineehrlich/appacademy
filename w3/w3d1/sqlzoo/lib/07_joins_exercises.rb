...
def andrews_films_and_leads
  ...
  # Andrews' played in.
  execute(<<-SQL)
  SELECT movies.title, a.name
  FROM movies
  JOIN castings ON movies.id = castings.movie_id
  JOIN actors ON actors.id = castings.actor_id
  JOIN castings c ON movies.id = c.movie_id
  JOIN actors a ON a.id = c.actor_id
  WHERE actors.name = 'Julie Andrews' AND c.ord = 1
...
def films_by_cast_size
  ...
  execute(<<-SQL)
  SELECT movies.title, COUNT(movies.id)
  FROM movies
  JOIN castings ON movies.id = castings.movie_id
  WHERE yr = 1978
  GROUP BY movies.id
  ORDER BY COUNT(movies.id) DESC, movies.title
  SQL
end
...
def colleagues_of_garfunkel
  ...
  JOIN castings c ON movies.id = c.movie_id
  JOIN actors a ON a.id = c.actor_id
  WHERE actors.name = 'Art Garfunkel' AND a.name <> 'Art Garfunkel'
